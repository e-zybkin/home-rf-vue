<template>
	<form class="form" @submit.prevent>
		<WallSizeInput 
			v-model:lengthSize="lengthSize"
			v-model:heightSize="heightSize"
		/>
		<OpeningBlock 
			:inputs="inputs" 
			@add="addNewInput"
	...
	export default {
		...
		methods: {
			...
			calculateRes() {
				...
				const zeroInOpening = this.inputs.filter(item => (
					item.widthSize === 0 && item.heightWallSize === 0
				)).length > 0;
				...
				if (zeroInWall || zeroInOpening) {
					...
					const openArea = this.inputs.map(item => {
